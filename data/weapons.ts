import type { WeaponDefinition } from "./equipment_types.ts";

// ---------------------------------------------------------------------------
// DATA – Weapons
// ---------------------------------------------------------------------------

// Helper to build compatible attachment lists
export const LONG_GUN_ATTACHMENTS = [
  "bayonet",
  "scope",
  "bipod",
  "strong-sling",
  "armor-piercing-rounds",
];
const REVOLVER_ATTACHMENTS = ["quickloader"];
const SHOTGUN_ATTACHMENTS = [
  "birdshot",
  "slug",
  "buck-n-ball",
];

export const WEAPONS: WeaponDefinition[] = [
  // ── Civilian / Generic ──
  {
    id: "colt-walker",
    name: "Colt Walker",
    type: "Black Powder Revolver",
    kind: "black-powder-revolver",
    nation: "Civilian",
    damage: "3",
    ammo: 6,
    rateOfFire: 1,
    weight: 2,
    pointCost: 0,
    traitIds: [
      "one-handed",
      "black-powder",
      "extremely-slow-reload",
      "overloaded-chamber",
    ],
    compatibleAttachmentIds: ["colt-walker-bullets"],
    reloadAmountOverride: 1,
    reloadTurns: 4,
  },
  {
    id: "colt-single-action-army",
    name: "Colt Single Action Army (Colt SAA)",
    type: "Single-action revolver",
    kind: "single-action-revolver",
    nation: "Civilian",
    damage: "2",
    ammo: 6,
    rateOfFire: 1,
    weight: 1,
    pointCost: 0,
    traitIds: ["one-handed", "agile", "loading-gate"],
    compatibleAttachmentIds: [],
    reloadAmountOverride: 1,
  },
  {
    id: "kolibri",
    name: "Kolibri",
    type: "Semiautomatic pistol",
    kind: "semiautomatic-pistol",
    nation: "Civilian",
    damage: "1",
    ammo: 6,
    rateOfFire: 2,
    weight: 1,
    pointCost: 0,
    traitIds: ["one-handed", "agile", "walking-fire", "kolibri", "concealable"],
    compatibleAttachmentIds: [],
  },
  {
    id: "derringer",
    name: "Derringer",
    type: "Break-action pistol",
    kind: "break-action-pistol",
    nation: "Civilian",
    damage: "2",
    ammo: 2,
    rateOfFire: 1,
    weight: 1,
    reloadAmountOverride: 1,
    pointCost: 0,
    traitIds: ["one-handed", "agile", "walking-fire", "concealable"],
    compatibleAttachmentIds: ["derringer-buckshot"],
  },
  {
    id: "double-barrel-shotgun",
    name: "Double barrel shotgun",
    type: "Shotgun",
    kind: "shotgun",
    nation: "Civilian",
    damage: "4 / 2",
    ammo: 2,
    rateOfFire: 1,
    weight: 2,
    pointCost: 0,
    traitIds: [
      "two-handed",
      "buckshot",
      "double-barrel-shotgun",
      "trench-sweeper",
      "double-fire",
    ],
    compatibleAttachmentIds: [
      "dbs-single-barrel",
      "dbs-sawn-off",
      "dbs-alofs-device",
      ...SHOTGUN_ATTACHMENTS,
    ],
    reloadAmountOverride: 1,
  },
  {
    id: "flintlock-musket",
    name: "Flintlock Musket",
    type: "Flintlock Smoothbore Musket",
    kind: "flintlock-musket",
    nation: "Civilian",
    damage: "4",
    ammo: 1,
    rateOfFire: 1,
    weight: 2,
    pointCost: 0,
    traitIds: ["two-handed", "extra-long-bayonet", "musket"],
    compatibleAttachmentIds: [
      "musket-rifled",
      "musket-breech-loader",
      ...LONG_GUN_ATTACHMENTS,
    ],
    reloadAmountOverride: 1,
    reloadTurns: 20,
  },
  {
    id: "bow",
    name: "Bow",
    type: "Bow & Arrow",
    kind: "bow",
    nation: "Civilian",
    damage: "3",
    ammo: 1,
    rateOfFire: 1,
    weight: 1,
    pointCost: 0,
    traitIds: [
      "two-handed",
      "recoverable-ammo-arrows",
      "draw-and-let-loose",
      "utterly-silent-bow",
    ],
    compatibleAttachmentIds: ["poison-tipped-arrows","longbow"],
    reloadAmountOverride: 1,
  },
  {
    id: "crossbow",
    name: "Crossbow",
    type: "Crossbow",
    kind: "crossbow",
    nation: "Civilian",
    damage: "2",
    ammo: 1,
    rateOfFire: 1,
    weight: 2,
    pointCost: 0,
    traitIds: [
      "two-handed",
      "recoverable-ammo-bolts",
      "utterly-silent-crossbow",
    ],
    compatibleAttachmentIds: ["chi-ko-nu","poison-tipped-bolts"],
    reloadAmountOverride: 1,
  },
  {
    id: "winchester-m1894",
    name: "Winchester M1887",
    type: "Lever-action rifle",
    kind: "lever-action-rifle",
    nation: "Civilian",
    damage: "2",
    ammo: 11,
    rateOfFire: 2,
    weight: 2,
    pointCost: 1,
    traitIds: ["two-handed", "walking-fire", "tubular-magazine-bullets"],
    compatibleAttachmentIds: ["strong-sling"],
    reloadAmountOverride: 1,
  },
  {
    id: "flamethrower",
    name: "Flamethrower",
    type: "Flamethrower",
    kind: "flamethrower",
    nation: "Any",
    damage: "3",
    ammo: 10,
    rateOfFire: 5,
    weight: 3,
    pointCost: 3,
    freeAccessoryIds: ["fuel-canister"],
    requiresMagazines: true,
    traitIds: [
      "two-handed",
      "restricted",
      "canister",
      "flamethrower",
      "volatile",
      "bulky-kit",
    ],
    compatibleAttachmentIds: [],
  },
  {
    id: "anti-tank-rifle",
    name: "Anti-tank rifle",
    type: "Anti-tank rifle",
    kind: "anti-tank-rifle",
    nation: "Any",
    damage: "8",
    ammo: 5,
    rateOfFire: 1,
    weight: 3,
    pointCost: 1,
    traitIds: [
      "two-handed",
      "bipod", "at-kickback", "at-inaccurate",
    ],
    compatibleAttachmentIds: [],
  },
  {
    id: "heavy-anti-tank-rifle",
    name: "Heavy anti-tank rifle",
    type: "Anti-tank rifle",
    kind: "anti-tank-rifle",
    nation: "Any",
    damage: "12",
    ammo: 5,
    rateOfFire: 1,
    weight: 5,
    pointCost: 3,
    reloadTurns: 3,
    traitIds: [
      "two-handed",
      "restricted",
      "tripod", "heavyat-kickback", "heavyat-inaccurate",
    ],
    compatibleAttachmentIds: [],
  },

  // ── British ──
  {
    id: "lee-enfield",
    name: "Lee-Enfield No.1 Mk III",
    type: "Bolt-action rifle",
    kind: "bolt-action-rifle",
    nation: "Britain",
    damage: "3",
    ammo: 10,
    rateOfFire: 1,
    weight: 2,
    pointCost: 0,
    traitIds: ["two-handed", "mad-minute"],
    compatibleAttachmentIds: [
      ...LONG_GUN_ATTACHMENTS,
      "lee-enfield-grenade-launcher",
    ],
  },
  {
    id: "webley-revolver",
    name: "Webley Revolver",
    type: "Double-action revolver",
    kind: "double-action-revolver",
    nation: "Britain",
    damage: "2",
    ammo: 6,
    rateOfFire: 1,
    weight: 1,
    pointCost: 0,
    traitIds: ["one-handed", "agile", "cylinder"],
    compatibleAttachmentIds: [...REVOLVER_ATTACHMENTS],
    reloadAmountOverride: 1,
  },
  {
    id: "lewis-gun",
    name: "Lewis Automatic Machine Gun",
    type: "Light Machinegun",
    kind: "light-machinegun",
    nation: "Britain",
    damage: "3",
    ammo: 45,
    rateOfFire: 7,
    weight: 3,
    pointCost: 1,
    traitIds: [
      "two-handed",
      "restricted",
      "bipod", 
      "lmg-recoil",
    ],
    compatibleAttachmentIds: ["lewis-gun-shield","lewis-extended-magazine"],
    discountFactionPerkIds: ["british-trench-raider", "harlem-hellfighter"],
    reloadTurns: 1,
  },
  {
    id: "vickers-machinegun",
    name: "Vickers machinegun",
    type: "Heavy Machinegun",
    kind: "heavy-machinegun",
    nation: "Britain",
    damage: "3",
    ammo: 250,
    rateOfFire: 11,
    weight: 4,
    pointCost: 3,
    traitIds: [
      "two-handed",
      "restricted",
      "tripod", 
      "hmg-recoil",
      "extra-heavy-magazines",
    ],
    compatibleAttachmentIds: [],
    freeAccessoryIds: ["vickers-magazine"],
    requiresMagazines: true,
    reloadTurns: 3,
  },

  // ── French ──
  {
    id: "lebel-m1886",
    name: "Lebel M1886",
    type: "Bolt-action rifle",
    kind: "bolt-action-rifle",
    nation: "France",
    damage: "3",
    ammo: 8,
    rateOfFire: 1,
    weight: 2,
    pointCost: 0,
    traitIds: ["two-handed", "tubular-magazine-bullets", "extra-long-bayonet"],
    compatibleAttachmentIds: [...LONG_GUN_ATTACHMENTS],
    reloadAmountOverride: 1,
  },
  {
    id: "modele-1892-revolver",
    name: "Modèle 1892 revolver",
    type: "Double-action revolver",
    kind: "double-action-revolver",
    nation: "France",
    damage: "2",
    ammo: 6,
    rateOfFire: 1,
    weight: 1,
    pointCost: 0,
    traitIds: ["one-handed", "agile", "cylinder"],
    compatibleAttachmentIds: [...REVOLVER_ATTACHMENTS],
    reloadAmountOverride: 1,
  },
  {
    id: "berthier-m1916",
    name: "Berthier M1916",
    type: "Bolt-action rifle",
    kind: "bolt-action-rifle",
    nation: "France",
    damage: "3",
    ammo: 5,
    rateOfFire: 1,
    weight: 2,
    pointCost: 0,
    traitIds: ["two-handed", "half-agile", "short-rifle", "en-block-clip"],
    compatibleAttachmentIds: [
      ...LONG_GUN_ATTACHMENTS,
      "berthier-chauchat-magazines",
    ],
  },
  {
    id: "hotchkiss_m1914",
    name: "Hotchkiss M1914",
    type: "Heavy Machinegun",
    kind: "heavy-machinegun",
    nation: "France",
    damage: "3",
    ammo: 30,
    rateOfFire: 11,
    weight: 4,
    pointCost: 3,
    traitIds: [
      "two-handed",
      "restricted",
      "tripod", "hmg-recoil",
    ],
    compatibleAttachmentIds: [],
  },
  {
    id: "ribeyrolles",
    name: "Ribeyrolles",
    type: "Assault rifle",
    kind: "assault-rifle",
    nation: "France",
    damage: "3",
    ammo: 25,
    rateOfFire: 5,
    weight: 2,
    pointCost: 3,
    traitIds: ["two-handed", "restricted"],
    discountFactionPerkIds: ["french-foreign-legion", "chasseurs-alpins"],
    compatibleAttachmentIds: [
      "bayonet",
      "strong-sling",
      "armor-piercing-rounds",
    ],
  },
  {
    id: "ruby-pistol",
    name: "'Ruby' pistol",
    type: "Semiautomatic pistol",
    kind: "semiautomatic-pistol",
    nation: "France",
    damage: "2",
    ammo: 9,
    rateOfFire: 2,
    weight: 1,
    pointCost: 1,
    traitIds: ["one-handed", "walking-fire"],
    compatibleAttachmentIds: [],
  },
  {
    id: "rsc-m1917",
    name: "RSC M1917",
    type: "Semiautomatic rifle",
    kind: "semiautomatic-rifle",
    nation: "France",
    damage: "3",
    ammo: 5,
    rateOfFire: 3,
    weight: 2,
    pointCost: 1,
    traitIds: ["two-handed", "walking-fire"],
    compatibleAttachmentIds: [
      ...LONG_GUN_ATTACHMENTS,
      "rsc-chauchat-magazines",
    ],
  },
  {
    id: "chauchat",
    name: "Chauchat",
    type: "Light-machinegun",
    kind: "light-machinegun",
    nation: "France",
    damage: "3",
    ammo: 20,
    rateOfFire: 7,
    weight: 3,
    pointCost: 1,
    traitIds: ["two-handed", "bipod", "lmg-recoil"],
    compatibleAttachmentIds: [],
  },

  // ── German ──
  {
    id: "gewehr-98",
    name: "Gewehr 98",
    type: "Bolt-action rifle",
    kind: "bolt-action-rifle",
    nation: "Germany",
    damage: "3",
    ammo: 5,
    rateOfFire: 1,
    weight: 2,
    pointCost: 0,
    traitIds: ["two-handed", "reliable"],
    compatibleAttachmentIds: [...LONG_GUN_ATTACHMENTS],
  },
  {
    id: "flieger-selbstlader-karabiner-1916",
    name: "Flieger Selbstlader Karabiner 1916",
    type: "Semiautomatic rifle",
    kind: "semiautomatic-rifle",
    nation: "Germany",
    damage: "3",
    ammo: 20,
    rateOfFire: 3,
    weight: 2,
    pointCost: 1,
    traitIds: ["two-handed"],
    compatibleAttachmentIds: [
      ...LONG_GUN_ATTACHMENTS,
    ],
  },
  {
    id: "c96-mauser",
    name: 'C96 "Mauser"',
    type: "Semiautomatic pistol",
    kind: "semiautomatic-pistol",
    nation: "Germany",
    damage: "2",
    ammo: 10,
    reloadAmountOverride: 10,
    rateOfFire: 2,
    weight: 1,
    pointCost: 1,
    traitIds: ["one-handed", "wasteful-reload", "walking-fire"],
    compatibleAttachmentIds: [
      "c96-extended-stock",
      "c96-extended-magazine",
      "c96-extended-barrel",
      "c96-9x25mm",
    ],
  },
  {
    id: "luger-p08",
    name: "Luger P08",
    type: "Semiautomatic pistol",
    kind: "semiautomatic-pistol",
    nation: "Germany",
    damage: "2",
    ammo: 8,
    rateOfFire: 2,
    weight: 1,
    pointCost: 1,
    traitIds: ["one-handed", "walking-fire"],
    compatibleAttachmentIds: [],
  },
  {
    id: "m1879-reichsrevolver",
    name: "M1879 Reichsrevolver",
    type: "Single action revolver",
    kind: "single-action-revolver",
    nation: "Germany",
    damage: "2",
    ammo: 6,
    rateOfFire: 1,
    weight: 1,
    pointCost: 0,
    traitIds: ["one-handed", "agile", "loading-gate"],
    compatibleAttachmentIds: [],
    reloadAmountOverride: 1,
  },
  {
    id: "mp-18",
    name: "Maschinenpistole 18 (MP-18)",
    type: "SMG",
    kind: "smg",
    nation: "Germany",
    damage: "2",
    ammo: 30,
    rateOfFire: 7,
    weight: 2,
    pointCost: 3,
    traitIds: ["two-handed", "restricted", "walking-fire"],
    compatibleAttachmentIds: ["bayonet", "strong-sling", "mp18-extended-magazine"],
    discountFactionPerkIds: ["sturmtruppen"],
  },
  {
    id: "mg08",
    name: "MG08",
    type: "Heavy Machinegun",
    kind: "heavy-machinegun",
    nation: "Germany",
    damage: "3",
    ammo: 250,
    rateOfFire: 11,
    weight: 4,
    pointCost: 3,
    traitIds: [
      "two-handed",
      "restricted",
      "tripod", "hmg-recoil",
      "extra-heavy-magazines",
    ],
    compatibleAttachmentIds: [],
    freeAccessoryIds: ["mg08-magazine"],
    requiresMagazines: true,
    reloadTurns: 3,
  },
  {
    id: "mg 08/15",
    name: "MG 08/15",
    type: "Light-machinegun",
    kind: "light-machinegun",
    nation: "Germany",
    damage: "3",
    ammo: 30,
    rateOfFire: 7,
    weight: 3,
    pointCost: 1,
    traitIds: ["two-handed", "bipod", "lmg-recoil","heavy-magazines"],
    freeAccessoryIds: ["mg08/15-magazine"],
    compatibleAttachmentIds: [],
    requiresMagazines: true,
  },

  // ── American ──
  {
    id: "m1903-springfield",
    name: "M1903 Springfield",
    type: "Bolt-action rifle",
    kind: "bolt-action-rifle",
    nation: "United States",
    damage: "3",
    ammo: 5,
    rateOfFire: 1,
    weight: 2,
    pointCost: 0,
    traitIds: ["two-handed", "reliable"],
    compatibleAttachmentIds: [
      ...LONG_GUN_ATTACHMENTS,
      "springfield-pedersen",
      "springfield-suppressor",
      "springfield-flyboy",
    ],
  },
  {
    id: "thompson",
    name: "Thompson",
    type: "SMG",
    kind: "smg",
    nation: "United States",
    damage: "2",
    ammo: 30,
    rateOfFire: 7,
    weight: 2,
    pointCost: 3,
    traitIds: ["two-handed", "restricted", "walking-fire"],
    compatibleAttachmentIds: [
      "thompson-drum-magazines",
      "bayonet",
      "strong-sling",
    ],
    discountFactionPerkIds: ["british-trench-raider", "harlem-hellfighter"],
  },
  {
    id: "burton-m1917-lmr",
    name: 'Burton M1917 LMR',
    type: "Assault Rifle",
    kind: "assault-rifle",
    nation: "United States",
    damage: "3",
    ammo: 40,
    reloadAmountOverride: 20,
    rateOfFire: 5,
    weight: 1,
    pointCost: 3,
    traitIds: ["two-handed", "restricted","dual-magazine"],
    discountFactionPerkIds: ["british-trench-raider", "harlem-hellfighter"],
    compatibleAttachmentIds: ["strong-sling","bayonet"],
  },
  {
    id: "browning_m1917",
    name: "Browning M1917",
    type: "Heavy Machinegun",
    kind: "heavy-machinegun",
    nation: "United States",
    damage: "3",
    ammo: 250,
    rateOfFire: 11,
    weight: 4,
    pointCost: 3,
    traitIds: [
      "two-handed",
      "restricted",
      "tripod", "hmg-recoil",
      "extra-heavy-magazines",
    ],
    compatibleAttachmentIds: [],
    freeAccessoryIds: ["browning-m1917-magazine"],
    requiresMagazines: true,
    reloadTurns: 3,
  },
  {
    id: "winchester-1897",
    name: 'Winchester Model 1897 "Trench Gun"',
    type: "Pump action shotgun",
    kind: "pump-action-shotgun",
    nation: "United States",
    damage: "4 / 2",
    ammo: 6,
    rateOfFire: 1,
    weight: 2,
    pointCost: 0,
    traitIds: [
      "two-handed",
      "buckshot",
      "tubular-magazine-shells",
      "trench-sweeper",
      "slam-fire",
    ],
    compatibleAttachmentIds: [
      "bayonet",
      "strong-sling",
      ...SHOTGUN_ATTACHMENTS,
    ],
    reloadAmountOverride: 1,
  },
  {
    id: "m1911",
    name: "M1911",
    type: "Semiautomatic pistol",
    kind: "semiautomatic-pistol",
    nation: "United States",
    damage: "2",
    ammo: 7,
    rateOfFire: 2,
    weight: 1,
    pointCost: 1,
    traitIds: ["one-handed", "walking-fire"],
    compatibleAttachmentIds: [],
  },
  {
    id: "colt-model-1917",
    name: "Colt M1917",
    type: "Double-action revolver",
    kind: "double-action-revolver",
    nation: "United States",
    damage: "2",
    ammo: 6,
    rateOfFire: 1,
    weight: 1,
    pointCost: 0,
    traitIds: ["one-handed", "agile", "cylinder"],
    compatibleAttachmentIds: [...REVOLVER_ATTACHMENTS, "m1917-moon-clips"],
    reloadAmountOverride: 1,
  },
  {
    id: "bar",
    name: "Browning Automatic Rifle (B.A.R)",
    type: "Light machinegun",
    kind: "light-machinegun",
    nation: "United States",
    damage: "3",
    ammo: 20,
    rateOfFire: 7,
    weight: 3,
    pointCost: 1,
    traitIds: ["two-handed", "bipod", "lmg-recoil", "select-fire"],
    compatibleAttachmentIds: [],
  },
  {
    id: "browning-auto-5",
    name: "Browning auto-5",
    type: "Semiautomatic shotgun",
    kind: "semiautomatic-shotgun",
    nation: "United States",
    damage: "4 / 2",
    ammo: 7,
    rateOfFire: 3,
    weight: 2,
    pointCost: 3,
    traitIds: [
      "two-handed",
      "restricted",
      "buckshot",
      "tubular-magazine-shells",
      "trench-sweeper",
    ],
    compatibleAttachmentIds: [
      "bayonet",
      "strong-sling",
      ...SHOTGUN_ATTACHMENTS,
    ],
    discountFactionPerkIds: ["british-trench-raider", "harlem-hellfighter"],
    reloadAmountOverride: 1,
  },

  // ── Japanese ──
  {
    id: "type-98-arisaka",
    name: "Type 38 Rifle",
    type: "Bolt-action rifle",
    kind: "bolt-action-rifle",
    nation: "Japan",
    damage: "3",
    ammo: 5,
    rateOfFire: 1,
    weight: 2,
    pointCost: 0,
    traitIds: ["two-handed", "6mm-arisaka", "extra-long-bayonet"],
    compatibleAttachmentIds: [...LONG_GUN_ATTACHMENTS],
  },
  {
    id: "type-38-carbine",
    name: "Type 38 Carbine",
    type: "Bolt-action rifle",
    kind: "bolt-action-rifle",
    nation: "Japan",
    damage: "3",
    ammo: 5,
    rateOfFire: 1,
    weight: 2,
    pointCost: 0,
    traitIds: ["two-handed", "6mm-arisaka", "half-agile", "short-rifle"],
    compatibleAttachmentIds: [...LONG_GUN_ATTACHMENTS],
  },
  {
    id: "type-44-bayonet-carbine",
    name: "Type 44 Cavalry Rifle",
    type: "Bolt-action rifle",
    kind: "bolt-action-rifle",
    nation: "Japan",
    damage: "3",
    ammo: 5,
    rateOfFire: 1,
    weight: 2,
    pointCost: 1,
    traitIds: ["two-handed", "6mm-arisaka", "half-agile", "integrated-bayonet"],
    compatibleAttachmentIds: [...LONG_GUN_ATTACHMENTS],
  },
  {
    id: "type-11",
    name: "Type 11",
    type: "Light-machinegun",
    kind: "light-machinegun",
    nation: "Japan",
    damage: "3",
    ammo: 30,
    reloadAmountOverride: 5,
    rateOfFire: 7,
    weight: 3,
    pointCost: 1,
    traitIds: [
      "two-handed",
      "bipod", "lmg-recoil",
      "6mm-arisaka",
      "stripper-clip-fed",
    ],
    compatibleAttachmentIds: [],
  },
  {
    id: "type_03",
    name: "Type 03",
    type: "Heavy Machinegun",
    kind: "heavy-machinegun",
    nation: "Japan",
    damage: "3",
    ammo: 30,
    rateOfFire: 11,
    weight: 4,
    pointCost: 3,
    traitIds: [
      "two-handed",
      "restricted",
      "6mm-arisaka",
      "tripod", "hmg-recoil",
    ],
    compatibleAttachmentIds: [],
  },
  {
    id: "type-26-revolver",
    name: "Type 26 Revolver",
    type: "Double-action revolver",
    kind: "double-action-revolver",
    nation: "Japan",
    damage: "2",
    ammo: 6,
    rateOfFire: 1,
    weight: 1,
    pointCost: 0,
    traitIds: ["one-handed", "agile", "cylinder"],
    compatibleAttachmentIds: [...REVOLVER_ATTACHMENTS],
    reloadAmountOverride: 1,
  },

  // ── Russian ──
  {
    id: "mosin-nagant",
    name: "Mosin-Nagant M1891 Rifle",
    type: "Bolt-Action Rifle",
    kind: "bolt-action-rifle",
    nation: "Russia",
    damage: "3",
    ammo: 5,
    rateOfFire: 1,
    weight: 2,
    pointCost: 0,
    traitIds: ["two-handed", "extra-long-bayonet"],
    compatibleAttachmentIds: [...LONG_GUN_ATTACHMENTS],
  },
  {
    id: "nagant-m1895-revolver",
    name: "Nagant M1895 Revolver",
    type: "Double-Action Revolver",
    kind: "double-action-revolver",
    nation: "Russia",
    damage: "2",
    ammo: 7,
    rateOfFire: 1,
    weight: 1,
    pointCost: 0,
    traitIds: ["one-handed", "agile", "loading-gate"],
    compatibleAttachmentIds: ["nagant-suppressor"],
    reloadAmountOverride: 1,
  },
  {
    id: "fedorov-avtomat",
    name: "Fedorov Avtomat",
    type: "Assault Rifle",
    kind: "assault-rifle",
    nation: "Russia",
    damage: "3",
    ammo: 25,
    rateOfFire: 5,
    weight: 2,
    pointCost: 3,
    traitIds: ["two-handed", "restricted", "6mm-arisaka"],
    compatibleAttachmentIds: [
      "bayonet",
      "strong-sling",
      "armor-piercing-rounds",
    ],
  },
  {
    id: "pm_m1910_maxim",
    name: "PM M1910 Maxim",
    type: "Heavy Machinegun",
    kind: "heavy-machinegun",
    nation: "Russia",
    damage: "3",
    ammo: 250,
    rateOfFire: 11,
    weight: 4,
    pointCost: 3,
    traitIds: [
      "two-handed",
      "restricted",
      "tripod", "hmg-recoil",
      "extra-heavy-magazines",
    ],
    compatibleAttachmentIds: ["maxim-gun-shield"],
    freeAccessoryIds: ["maxim-m1910-magazine"],
    requiresMagazines: true,
    reloadTurns: 3,
  },
  {
    id: "madsen_light_machinegun",
    name: "Madsen light machinegun",
    type: "Light-machinegun",
    kind: "light-machinegun",
    nation: "Russia",
    damage: "3",
    ammo: 40,
    rateOfFire: 7,
    weight: 3,
    pointCost: 1,
    traitIds: [
      "two-handed",
      "bipod", "lmg-recoil",
      "overhead-magazine",
    ],
    compatibleAttachmentIds: [],
  },
  {
    id: "winchester-m1895",
    name: "Winchester M1895",
    type: "Lever-action rifle",
    kind: "lever-action-rifle",
    nation: "Russia",
    damage: "3",
    ammo: 5,
    rateOfFire: 2,
    weight: 2,
    pointCost: 1,
    traitIds: ["two-handed", "walking-fire"],
    compatibleAttachmentIds: [...LONG_GUN_ATTACHMENTS],
  },

  // ── Austro-Hungarian ──
  {
    id: "mannlicher-m1895",
    name: "Mannlicher M1888",
    type: "Bolt-action rifle",
    kind: "bolt-action-rifle",
    nation: "Austria-Hungary",
    damage: "3",
    ammo: 5,
    rateOfFire: 1,
    weight: 1,
    pointCost: 0,
    traitIds: ["two-handed", "extra-long", "en-block-clip"],
    compatibleAttachmentIds: [...LONG_GUN_ATTACHMENTS],
  },
  {
    id: "mannlicher-m1895-stutzen",
    name: "Mannlicher M1895 Stutzen",
    type: "Bolt-action rifle",
    kind: "bolt-action-rifle",
    nation: "Austria-Hungary",
    damage: "3",
    ammo: 5,
    rateOfFire: 1,
    weight: 1,
    pointCost: 0,
    traitIds: ["two-handed", "half-agile", "short-rifle", "en-block-clip"],
    compatibleAttachmentIds: [...LONG_GUN_ATTACHMENTS],
  },
  {
    id: "schwarzlose_m07/12",
    name: "Schwarzlose M.07/12",
    type: "Heavy Machinegun",
    kind: "heavy-machinegun",
    nation: "Austria-Hungary",
    damage: "3",
    ammo: 250,
    rateOfFire: 11,
    weight: 4,
    pointCost: 3,
    traitIds: [
      "two-handed",
      "restricted",
      "tripod", "hmg-recoil",
      "extra-heavy-magazines",
    ],
    compatibleAttachmentIds: [],
    freeAccessoryIds: ["schwarzlose-m07/12-magazine"],
    requiresMagazines: true,
    reloadTurns: 3,
  },
  {
    id: "steyr-m1912",
    name: "Steyr M1912",
    type: "Semiautomatic pistol",
    kind: "semiautomatic-pistol",
    nation: "Austria-Hungary",
    damage: "2",
    ammo: 8,
    reloadAmountOverride: 8,
    rateOfFire: 2,
    weight: 1,
    pointCost: 1,
    traitIds: ["one-handed", "walking-fire"],
    compatibleAttachmentIds: [
      "steyr-automatic-fire",
      "steyr-extended-magazine",
      "steyr-extended-stock",
    ],
  },

  // ── Italian ──
  {
    id: "carcano-m91",
    name: 'Fucile M91 "Carcano"',
    type: "Bolt-action rifle",
    kind: "bolt-action-rifle",
    nation: "Italy",
    damage: "3",
    ammo: 6,
    rateOfFire: 1,
    weight: 2,
    pointCost: 0,
    traitIds: ["two-handed", "en-block-clip", "6-5mm-carcano"],
    compatibleAttachmentIds: [...LONG_GUN_ATTACHMENTS],
  },
  {
    id: "Beretta-Modelo-1918",
    name: "Beretta Modelo 1918",
    type: "Semiautomatic rifle",
    kind: "semiautomatic-rifle",
    nation: "Italy",
    damage: "3",
    ammo: 25,
    rateOfFire: 3,
    weight: 2,
    pointCost: 1,
    traitIds: ["two-handed", "overhead-magazine"],
    discountFactionPerkIds: ["italian-veteran"],
    compatibleAttachmentIds: [...LONG_GUN_ATTACHMENTS],
  },
  {
    id: "Carabinetta-Automatica-O.V.P",
    name: 'Carabinetta Automatica "O.V.P"',
    type: "SMG",
    kind: "smg",
    nation: "Italy",
    damage: "2",
    ammo: 25,
    rateOfFire: 7,
    weight: 2,
    pointCost: 3,
    traitIds: ["two-handed", "restricted", "overhead-magazine", "walking-fire"],
    discountFactionPerkIds: ["italian-veteran"],
    compatibleAttachmentIds: ["bayonet", "strong-sling"],
  },
  {
    id: "villar-perosa-m1915",
    name: 'Villar Perosa M1915',
    type: "Light Machinegun",
    kind: "light-machinegun",
    nation: "Italy",
    damage: "2",
    ammo: 50,
    reloadAmountOverride: 25,
    rateOfFire: 7,
    weight: 2,
    pointCost: 3,
    traitIds: ["two-handed", "restricted", "bipod", "lmg-recoil", "overhead-magazine","smg-hybrid","dual-magazine"],
    discountFactionPerkIds: ["italian-veteran",],
    compatibleAttachmentIds: ["strong-sling",],
  },
  {
    id: "fiat-revelli-modello-1914",
    name: "Fiat–Revelli Modello 1914",
    type: "Heavy Machinegun",
    kind: "heavy-machinegun",
    nation: "Italy",
    damage: "3",
    ammo: 250,
    rateOfFire: 11,
    weight: 4,
    pointCost: 3,
    traitIds: [
      "two-handed",
      "restricted",
      "tripod", "hmg-recoil",
      "extra-heavy-magazines",
      "6-5mm-carcano",
    ],
    compatibleAttachmentIds: [],
    freeAccessoryIds: ["fiat-revelli-modello-1914"],
    requiresMagazines: true,
    reloadTurns: 3,
  },
  {
    id: "bodeo-1889",
    name: "Bodeo Model 1889 revolver",
    type: "Double-action revolver",
    kind: "double-action-revolver",
    nation: "Italy",
    damage: "2",
    ammo: 6,
    rateOfFire: 1,
    weight: 1,
    pointCost: 0,
    traitIds: ["one-handed", "loading-gate", "agile"],
    compatibleAttachmentIds: [],
    reloadAmountOverride: 1,
  },

  // ── Swiss ──
  {
    id: "schmidt-rubin-1911",
    name: "Schmidt–Rubin Model 1911",
    type: "Bolt action rifle",
    kind: "bolt-action-rifle",
    nation: "Switzerland",
    damage: "3",
    ammo: 6,
    rateOfFire: 1,
    weight: 2,
    pointCost: 0,
    traitIds: ["two-handed", "magazine-fed"],
    compatibleAttachmentIds: [...LONG_GUN_ATTACHMENTS],
    freeAccessoryIds: ["schmidt-rubin-magazine"],
  },
  {
    id: "ordonnanzrevolver-1882",
    name: "Ordonnanzrevolver 1882",
    type: "Double-action revolver",
    kind: "double-action-revolver",
    nation: "Switzerland",
    damage: "2",
    ammo: 6,
    rateOfFire: 1,
    weight: 1,
    pointCost: 0,
    traitIds: ["one-handed", "agile", "cylinder"],
    compatibleAttachmentIds: [...REVOLVER_ATTACHMENTS],
    reloadAmountOverride: 1,
  },
  {
    id: "mg11",
    name: "MG11",
    type: "Heavy Machinegun",
    kind: "heavy-machinegun",
    nation: "Switzerland",
    damage: "3",
    ammo: 250,
    rateOfFire: 11,
    weight: 4,
    pointCost: 3,
    traitIds: [
      "two-handed",
      "restricted",
      "tripod", "hmg-recoil",
      "extra-heavy-magazines",
    ],
    compatibleAttachmentIds: [],
    freeAccessoryIds: ["mg11-magazine"],
    requiresMagazines: true,
    reloadTurns: 3,
  },
  {
    id: "stamm-sauer-1916",
    name: "Stamm Sauer 1916",
    type: "Light-machinegun",
    kind: "light-machinegun",
    nation: "Switzerland",
    damage: "3",
    ammo: 20,
    rateOfFire: 7,
    weight: 3,
    pointCost: 1,
    traitIds: ["two-handed", "bipod", "lmg-recoil"],
    compatibleAttachmentIds: [],
  },
  {
    id: "furrer-mp19",
    name: "Furrer MP19",
    type: "SMG",
    kind: "smg",
    nation: "Switzerland",
    damage: "2",
    ammo: 40,
    rateOfFire: 7,
    weight: 2,
    pointCost: 3,
    traitIds: ["two-handed", "restricted", "walking-fire"],
    compatibleAttachmentIds: ["bayonet", "strong-sling"],
    discountFactionPerkIds: ["king-s-royal-army-pf"],
  },
];

export const WEAPONS_BY_ID = new Map(
  WEAPONS.map((w) => [w.id, w]),
);
